...
public class BaiduTranslateService:ITranslateService
{
    ...
    public async Task<TranslateResponse> TranslateAsync(TranslateRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var salt = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 10);
        ...
        try
        {
            ...
            var translateResponse = JsonSerializer.Deserialize<TranslateResponse>(responseContent, SourceGenerationContext.Default.TranslateResponse);
